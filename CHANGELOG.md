...
signatures; a 1.0.x integration that supplies `WithPartnerToken` keeps working.
...
- **`client.Auth`** with three methods:
  - `Connect` — `POST /general/connectApi`, public. Exchanges the portal
    ...
    `TwoFactorRequired` with the server's challenge instead of failing.
  ...
  - `Disconnect` — `POST /general/disconnectApi`. Revokes the access token and
    ...
    server-side.
- **`WithCredentials`** option, used by `Connect` and by the silent refresh.
...
- `RefreshTokenStore`, the optional `TokenStore` extension that carries it.
  `NewInMemoryTokenStorePair` seeds a store with both tokens;
  `NewInMemoryTokenStore` still takes just the access token.
